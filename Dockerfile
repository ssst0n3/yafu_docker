...
RUN mkdir -p /ggnfs && unzip -d /ggnfs gnfs-lasieve_2010-11-18.zip && chmod +x /ggnfs -R
...
RUN sed -i 's@/users/buhrow/src/c/gmp_install/gmp-6.2.0/lib/libgmp.a@/usr/lib/x86_64-linux-gnu/libgmp.a@g' Makefile && \
    ...
    sed -i 's@/home/nyancat/Tools/cado-nfs/@/cado-nfs/@g' yafu.ini && \
    # sed -i '/^tune_info.*/d' yafu.ini && \
    sed -i 's@ecm_path=..\\gmp-ecm\\bin\\x64\\Release\\ecm.exe@ecm_path=/usr/bin/ecm@g' yafu.ini
RUN make NFS=1

FROM debian:11 AS release
RUN sed -i "s@http://.*.debian.org@http://repo.huaweicloud.com@g" /etc/apt/sources.list
RUN apt update && apt install -y gmp-ecm libecm1-dev
COPY --from=yafu /ggnfs /ggnfs
COPY --from=yafu /yafu/yafu.ini /yafu.ini
